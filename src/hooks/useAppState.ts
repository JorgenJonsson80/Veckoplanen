...
export function useAppState(user: User | null) {
  ...
  const { likelyEmptyItems, suggestedRebuys, budgetHistory, budgetSummary, monthlySummary, toggleItem, saveWeeklyList, addExtraItem, removeExtraItem, hideIngredient, restoreIngredients, clearChecked, setBudget, setWeeklySpend, logAteOut, removeAteOut } = useShoppingList(state, updateState, ingredientMap, categories)
