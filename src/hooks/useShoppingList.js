...
export function useShoppingList(state, updateState, ingredientMap, categories) {
  ...
  const toggleItem = useCallback((itemName, category) => {
  ...
  const saveWeeklyList = useCallback((allItems, meals) => {
  ...
  const addExtraItem = useCallback((name, catId) => {
  ...
  const setWeeklySpend = useCallback((value) => {
